...
print train.describe() #describe gives numeric data
...
print train.describe(include="all").T.unique  #gives report of all 12 columns, t.unique helps in extracting the column
...
print
print train.sample(10) #any random 10 values
...
#Alphanumeric Features: Name, Ticket, Cabin #text
...
#4)The Embarked feature is missing only 2 passengers,
#  which should be relatively harmless.

#check for any other unusable values

print
print pd.isnull(train).sum() # to check for null
...
sbn.barplot(x="Sex", y="Survived", data=train) #check for co-relation
plt.show()
...
#normalize tells the value without percentage
...
#Some Observations from above output
#------------------------------------
#In general, its clear that people with more siblings or spouses aboard were less likely to survive.
# However, contrary to expectations, people with no siblings or spouses
...
train["Age"] = train["Age"].fillna(-0.5) #fillna- searches for null value
test["Age"] = test["Age"].fillna(-0.5) #-sign to check the missing value as indication

bins = [-1, 0, 5, 12, 18, 24, 35, 60, np.inf]
labels = ['Unknown', 'Baby', 'Child', 'Teenager', 'Student', 'Young Adult', 'Adult', 'Senior']
train['AgeGroup'] = pd.cut(train["Age"], bins, labels=labels) #keep the value in cztegories according to bin
...
for dataset in combine:
    dataset['Title'] = dataset['Name'].str.extract(' ([A-Za-z]+)\.', expand=False) #expand=if found dont search again
                                                                                  #/=escape sequence(change the meaning e.g, .)
                                                                                  #.=any one but here text
...
print pd.crosstab(train['Title'], train['Sex']) #report of sex on the basis of title
...
for dataset in combine:
    dataset['Title'] = dataset['Title'].replace(['Lady', 'Capt', 'Col','Don', 'Dr', 'Major', 'Rev', 'Jonkheer', 'Dona'], 'Rare') #rare=rare ocurring group
    dataset['Title'] = dataset['Title'].replace(['Countess', 'Sir'], 'Royal')
    dataset['Title'] = dataset['Title'].replace('Mlle', 'Miss')
    dataset['Title'] = dataset['Title'].replace('Ms', 'Miss')
    dataset['Title'] = dataset['Title'].replace('Mme', 'Mrs') #replace(source, target)
...
title_mapping = {"Mr": 1, "Miss": 2, "Mrs": 3, "Master": 4, "Royal": 5, "Rare": 6} #map function to convert text to digit
for dataset in combine:
    dataset['Title'] = dataset['Title'].map(title_mapping)
    dataset['Title'] = dataset['Title'].fillna(0) #no title in name fill with zero
...
print "\n\n********   train[AgeGroup][x] :  \n\n"
for x in range(10) :
    print train["AgeGroup"][x] #two bracket= 1st: the column 2nd: its value
...
for x in range(len(test["Fare"])):
    if pd.isnull(test["Fare"][x]):
        ...
        test["Fare"][x] = round(train[ train["Pclass"] == pclass ]["Fare"].mean(), 2) #round off upto 2 decimal places

#map Fare values into groups of numerical values
train['FareBand'] = pd.qcut(train['Fare'], 4, labels = [1, 2, 3, 4]) #divide fare according to categories
...
predictors = train.drop(['Survived', 'PassengerId'], axis=1)#historic x
target = train["Survived"]                                  #historic y
...
#OUTPUT:-
#MODEL-1: Accuracy of LogisticRegression :  79.19
...
output.to_csv('submission.csv', index=False)#no rows
